...
export default function CouponsPage() {
  ...
  const filtered = coupons.filter(c => {
    const isExpired = new Date(c.expiry) < now || (c.usageLimit && c.usedCount >= c.usageLimit) || !c.isActive;
          ...
          <AnimatePresence mode="popLayout">
            {isLoading ? (
               Array(4).fill(0).map((_, i) => (
                 <Skeleton key={i} className="h-[180px] rounded-[2.5rem] bg-white dark:bg-white/5" />
               ))
            ) : filtered.length === 0 ? (
              <motion.div 
                initial={{ opacity: 0 }} 
              ...
              filtered.map((coupon, i) => {
                const isExpired = new Date(coupon.expiry) < now || (coupon.usageLimit && coupon.usedCount >= coupon.usageLimit) || !coupon.isActive;
                    ...
                    <div className={cn(
                      ...
                      <div className="relative">
                        <p className={cn(
                          ...
                          {coupon.discountType === "PERCENTAGE" ? `${Number(coupon.discount)}%` : `₹${Number(coupon.discount)}`}
                        </p>
                        <p className={cn(
                        ...
                        <div className="flex items-center justify-between mb-4">
                          ...
                          {isExpired && (
                            <Badge variant="ghost" className="text-[10px] font-black uppercase text-red-400 p-0">
                             {!coupon.isActive ? "Inactive" : isUsed ? "Used Up" : "Expired"}
                            </Badge>
                          )}
                        </div>
                        <h4 className="text-[13px] font-black text-slate-800 dark:text-slate-200 uppercase tracking-tight leading-snug">
                           {coupon.discountType === "PERCENTAGE" ? `${Number(coupon.discount)}% discount on your next booking.` : `Flat ₹${Number(coupon.discount)} off on your order total.`}
                        </h4>
                        <div className="flex flex-wrap items-center gap-4 mt-4 text-[11px] font-bold text-slate-400">
                           <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-primary/50" /> Ends: {formatDate(coupon.expiry)}</span>
                           {coupon.usageLimit && <span className="flex items-center gap-1.5"><Tag className="w-3.5 h-3.5 text-primary/50" /> Limit: {coupon.usedCount}/{coupon.usageLimit}</span>}
                        </div>
                      </div>
