...
export default function HomePage() {
  ...
  const trendingSearches = tHome.raw("trending_list") as string[];
  const localizedPromoOffers = tHome.raw("promo_offers") as any[];
  ...
  const [isLoadingMostSold, setIsLoadingMostSold] = useState(true);
  ...
  useEffect(() => {
    const safeParse = (val: any) => {
      ...
      if (typeof val === 'object' && val.d) {
        try {
          return parseFloat(`${val.s * val.d[0]}.${val.d.slice(1).join('') || '0'}`);
        } catch (e) { return 0; }
      }
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (json.status === "success") {
          setDynamicCategories(json.data.categories || []);
        }
      } catch (error) {
    ...
    const fetchHomeOffers = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/v1/home-offers`,
          { headers: { "Accept-Language": locale } }
        );
        const json = await response.json();
        if (json.status === "success") {
          setDynamicOffers(json.data.offers || []);
        }
      } catch (error) {
    ...
    const fetchBanners = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/v1/banners`,
          { headers: { "Accept-Language": locale } }
        );
        const json = await response.json();
        if (json.status === "success") {
          setDynamicBanners(json.data.banners.filter((b: any) => b.isActive) || []);
        }
      } catch (error) {
    ...
    const fetchBrands = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/v1/brands`);
        const json = await response.json();
        if (json.status === "success") {
          setDynamicBrands(json.data.brands.filter((b: any) => b.isActive) || []);
        }
      } catch (error) {
    ...
    const fetchPackages = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/v1/products?type=BUNDLE&limit=10`,
          { headers: { "Accept-Language": locale } }
        );
        const json = await response.json();
        if (json.status === "success") {
          const mapped = json.data.products.map((p: any) => {
            ...
            const catStr = typeof catName === 'object' ? (catName[locale] || catName.en || Object.values(catName)[0]) : (catName || "Premium Selection");

            return {
              id: p.id,
              name: nameStr || "Untitled Package",
              ...
              category: catStr,
              vendorId: "v-premium",
              vendorName: "Ayur Pooja",
              badge: p.badge || (safeParse(p.mrp) > safeParse(p.price) ? `SAVE ${Math.round(((safeParse(p.mrp) - safeParse(p.price)) / safeParse(p.mrp)) * 100)}%` : null),
              inStock: p.stock > 0,
              ...
              itemsCount: p.bundleItems?.length || 0
            };
          });
    ...
    const fetchMostSold = async () => {
        try {
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/v1/products?type=STANDARD&limit=12`,
            { headers: { "Accept-Language": locale } }
          );
          const json = await response.json();
          if (json.status === "success") {
            const mapped = json.data.products.map((p: any) => {
              const nameStr = typeof p.name === 'object' ? (p.name[locale] || p.name.en || Object.values(p.name)[0]) : p.name;
              const catName = p.category?.name;
              const catStr = typeof catName === 'object' ? (catName[locale] || catName.en || Object.values(catName)[0]) : (catName || "Groceries");
              ...
              return {
                ...
                images: p.images || (p.image ? [p.image] : []),
                category: catStr,
                badge: p.badge || (safeParse(p.mrp) > safeParse(p.price) ? `${Math.round(((safeParse(p.mrp) - safeParse(p.price)) / safeParse(p.mrp)) * 100)}% OFF` : null),
                inStock: p.stock > 0,
              };
            });
            setMostSoldProducts(mapped);
          }
        } catch (error) {
          console.error("Failed to fetch most sold products", error);
        } finally {
          setIsLoadingMostSold(false);
        }
      };
  ...
  const promoOffers = dynamicOffers.length > 0 ? dynamicOffers : [
    {
      title: "Pharmacy\nDelivered",
    ...
    <main className="min-h-screen bg-white dark:bg-[#08090a] text-foreground overflow-hidden">
          ...
          <div className="max-w-4xl w-full">
            ...
            <div className="mb-6 md:mb-10">
              ...
              >
                ...
                <span className="text-primary italic">{t("title").split(' ').slice(3).join(' ')}</span>
              </motion.h1>
              <motion.p
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
            ...
            <div className="relative w-full min-h-20 md:min-h-28 flex flex-col items-center">
              ...
              <div className="flex flex-col items-center w-full mt-6 md:mt-10 space-y-3 px-2 overflow-visible">
                ...
                >
                  ...
                  <p className="text-[10px] md:text-xs font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">{tHome("trending")}</p>
                </motion.div>

                <div className="flex flex-nowrap md:flex-wrap overflow-x-auto w-full md:justify-center gap-2 pb-2 md:pb-0 no-scrollbar snap-x px-4 md:px-0">
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row md:flex-wrap lg:flex-nowrap gap-4 md:gap-6 justify-center">
            {promoOffers.map((offer, i) => {
              // Priority: If dynamicOffers exists, we use it directly as it's already localized from backend
              // Otherwise, we fall back to static localizedPromoOffers
              const isDynamic = dynamicOffers.length > 0;
              const displayTitle = isDynamic ? offer.title : (localizedPromoOffers[i]?.title || offer.title);
              const displaySubtitle = isDynamic ? offer.subtitle : (localizedPromoOffers[i]?.subtitle || offer.subtitle);
              const displayExplore = isDynamic ? tHome("explore_now") : (localizedPromoOffers[i]?.explore || tHome("explore_now"));
              ...
              return (
                ...
                >
                  <motion.div
                    ...
                    className={`relative overflow-hidden rounded-[2rem] ${offer.color} p-6 md:p-10 flex flex-col justify-between group cursor-pointer transition-transform hover:-translate-y-2 shadow-sm border border-slate-100 dark:border-white/5 h-[180px] md:h-[240px]`}
                  >
      ...
      <section className="container mx-auto px-4 py-8 md:py-16 overflow-hidden">
        ...
        <div className="hidden md:flex overflow-x-auto scrollbar-hide gap-4 md:flex-wrap md:gap-6 shrink-0 md:justify-center items-start pb-4">
          {isLoadingCats ? (
            Array(8).fill(0).map((_, i) => (
              <div key={i} className="flex flex-col items-center shrink-0 w-[130px] animate-pulse">
                <div className="relative w-full aspect-[3/4.2] rounded-t-full rounded-b-[2rem] bg-slate-100 dark:bg-slate-800 p-2.5" />
              </div>
            ))
          ) : (
            dynamicCategories.map((cat) => (
              <Link key={cat.id} href={`/products?category=${cat.slug}`} className="group flex flex-col items-center shrink-0 w-[100px] md:w-[130px]">
                  ...
                  <div className="h-12 md:h-14 flex flex-col items-center justify-center shrink-0 w-full relative overflow-hidden">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center transition-all duration-300 translate-y-full group-hover:translate-y-0 opacity-0 group-hover:opacity-100">
                      <span className="text-[9px] md:text-[10px] font-bold text-primary uppercase tracking-widest flex items-center gap-1 group-hover:scale-105 transition-transform duration-300 delay-100">
                        {tHome("explore_cat")} <ArrowRight className="w-3 h-3" />
                      </span>
                    </div>
                  </div>
                </div>
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tighter">
              {tHome("curated_gift_boxes").split(' ').slice(0, 1).join(' ')} <span className="text-primary italic">{tHome("curated_gift_boxes").split(' ').slice(1).join(' ')}.</span>
            </h2>
            <p className="text-sm text-slate-500 font-medium tracking-tight">{tHome("curated_subtitle")}</p>
          </div>
          <Link href="/products?type=BUNDLE">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4">
          {isLoadingPackages ? (
            ...
            dynamicPackages.length > 0 ? (
              ...
              productPackages.slice(0, 5).map((pkg) => (
                <ProductCard key={pkg.id} product={pkg} layout="square" />
              ))
            )
          )}
      ...
      {/* ── DYNAMIC PREMIUM PROMO BANNERS ── */}
      {dynamicBanners.length > 0 ? (
        dynamicBanners.map((banner, idx) => (
          <section key={banner.id} className="w-full pb-12 md:pb-16 relative group px-4 md:px-0">
            <motion.div
            ...
            >
              <Link href={banner.link || "/products"}>
                <div className={cn("relative w-full rounded-[2rem] overflow-hidden min-h-[220px] md:h-[280px] flex items-center mt-8 group/banner border border-border/40 shadow-sm hover:shadow-xl transition-all duration-500", banner.color || "bg-primary/5")}>
                  {/* Clean Background Accent */}
                  <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-white/30 to-transparent pointer-events-none" />

                  <div className="relative z-10 w-full h-full flex items-center px-6 md:px-12 py-8 md:py-0">

                    {/* Left Content (Clean Typography stretching full height) */}
                    <div className="w-[60%] md:w-[50%] h-full flex flex-col justify-between py-6 md:py-10 z-20 pr-4">
                      <div className="space-y-3 md:space-y-4">
                        {banner.subtitle && (
                          <Badge className="bg-slate-900 text-white rounded-md px-3 py-1 font-bold text-[10px] md:text-xs uppercase tracking-widest border-none shadow-md mb-2">
                            {banner.subtitle}
                          </Badge>
                        )}
                        <h3 className="text-3xl sm:text-4xl md:text-5xl lg:text-5xl font-black text-slate-900 leading-[1.05] tracking-tight uppercase">
                      ...
                      <div>
                        <button className="flex items-center gap-2 group/btn">
                          <span className="font-black text-[11px] md:text-sm uppercase tracking-widest text-slate-900 underline-offset-4 group-hover/btn:underline decoration-2">
                            {tHome("explore_now")}
                          </span>
                          <div className="w-8 h-8 rounded-full bg-slate-900 text-white flex items-center justify-center group-hover/btn:translate-x-1 transition-transform">
                    ...
                    <div
                      className="absolute right-0 top-0 w-[50%] md:w-[60%] h-full pointer-events-none z-10 rounded-r-[2rem]"
                      style={{ WebkitMaskImage: "linear-gradient(to right, transparent 0%, black 15%)", maskImage: "linear-gradient(to right, transparent 0%, black 15%)" }}
                    >
                      <div className="relative w-full h-full">
                        <Image
                          src={banner.image}
                          alt={banner.title}
                          fill
                          className="object-cover object-center group-hover/banner:scale-105 transition-transform duration-700 ease-out"
                          priority
                        />
                      </div>
                    </div>
        ...
        /* Fallback / Loading Empty State or Hardcoded Default if Needed */
        <div className="h-4" />
      )}
      ...
      <section className="container mx-auto px-4 py-12 pb-20">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tighter uppercase">{tHome("most_sold").split(' ').slice(0, 2).join(' ')} <span className="text-primary italic">{tHome("most_sold").split(' ').slice(2).join(' ')}</span></h2>
            <p className="text-slate-500 dark:text-slate-400 text-xs md:text-sm mt-1 font-medium italic underline underline-offset-4 decoration-primary/30">{tHome("community_favorites")}</p>
          </div>
          <Link href="/products">
            <Button variant="ghost" className="text-primary hover:opacity-80 gap-2 font-bold uppercase tracking-widest text-[9px] md:text-[10px]">
              {tHome("see_all")} <ChevronRight className="w-4 h-4" />
            </Button>
          </Link>
        </div>
          ...
          {isLoadingMostSold ? (
            ...
            mostSoldProducts.length > 0 ? (
              ...
              <div className="col-span-full py-20 text-center opacity-40">
                ...
                <p className="text-sm font-black uppercase tracking-widest">Discovering Community Favorites...</p>
              </div>
            )
          )}
        </div>
        ...
        <section className="bg-primary/[0.02] dark:bg-primary/[0.01] py-12 md:py-20 border-y border-slate-100 dark:border-white/10 overflow-hidden">
          ...
          <div className="relative flex overflow-hidden">
            ...
            <motion.div
              ...
              transition={{
                ease: "linear",
                duration: dynamicBrands.length * 3 > 15 ? dynamicBrands.length * 3 : 20,
                repeat: Infinity
              }}
            >
              {[...dynamicBrands, ...dynamicBrands, ...dynamicBrands].map((brand, i) => {
                ...
                try {
                  if (brand.name.startsWith('{')) {
                    const parsed = JSON.parse(brand.name);
                    brandName = parsed[locale] || parsed.en || "Unnamed";
                  }
                } catch (e) { }
                ...
                return (
                  <div key={`${brand.name}-${i}`} className="flex flex-col items-center gap-2 group cursor-pointer transition-all duration-500 flex-shrink-0">
                    ...
                    <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 group-hover:text-primary transition-colors">{brandName}</span>
                  </div>
                )
              })}
            </motion.div>
        ...
        <div className="relative overflow-hidden rounded-[2rem] md:rounded-none bg-primary/[0.03] dark:bg-primary/[0.02] border border-primary/5 dark:border-primary/10 md:border-transparent p-6 sm:p-8 md:p-16">
          ...
          <div className="relative flex flex-col md:flex-row items-center gap-8 md:gap-12">
            ...
            <div className="flex-1 text-center md:text-left mt-4 md:mt-0">
              <Badge className="mb-4 md:mb-6 bg-primary/10 text-primary border-primary/20 px-3 py-1 rounded-full uppercase tracking-widest text-[9px] md:text-[10px] font-black">{tHome("mobile_exp")}</Badge>
              <h2 className="text-2xl sm:text-3xl md:text-5xl font-black text-slate-900 dark:text-white mb-3 md:mb-6 leading-[1.1] tracking-tighter">
                {tHome("shop_on_go_title").split(' ').slice(0, 5).join(' ')} <br className="hidden sm:block" />
                <span className="text-primary">{tHome("shop_on_go_title").split(' ').slice(5).join(' ')}</span>
              </h2>
              ...
              <div className="flex flex-col sm:flex-row justify-center md:justify-start gap-3 md:gap-4">
                ...
                <button className="flex items-center justify-center gap-3 bg-slate-950 dark:bg-white text-white dark:text-slate-950 px-5 py-3 md:px-6 md:py-3 rounded-2xl hover:bg-primary dark:hover:bg-primary hover:text-white dark:hover:text-white w-full sm:w-auto shadow-sm">
                  ...
                  <div className="text-left">
                    <div className="text-[9px] md:text-[10px] font-bold uppercase tracking-widest opacity-70">{tHome("download_on")}</div>
                    <div className="text-sm md:text-lg font-black leading-tight">{tHome("app_store")}</div>
                  </div>
                </button>
                ...
                <button className="flex items-center justify-center gap-3 bg-slate-950 dark:bg-white text-white dark:text-slate-950 px-5 py-3 md:px-6 md:py-3 rounded-2xl hover:bg-primary dark:hover:bg-primary hover:text-white dark:hover:text-white transition-all shadow-md active:scale-95 group w-full sm:w-auto">
                  ...
                  <div className="text-left">
                    <div className="text-[9px] md:text-[10px] font-bold uppercase tracking-widest opacity-70">{tHome("get_it_on")}</div>
                    <div className="text-sm md:text-lg font-black leading-tight">{tHome("play_store")}</div>
                  </div>
                </button>
              </div>
            </div>
