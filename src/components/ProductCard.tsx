...
export default function ProductCard({ 
  ...
  return (
    <div className="bg-white dark:bg-[#0c0d0e] rounded-2xl border border-slate-200 dark:border-white/5 p-2 hover:shadow-xl transition-all duration-500 overflow-hidden flex flex-col h-full group/card relative">
      {/* Product Image Carousel */}
      <div className="relative aspect-square w-full rounded-xl overflow-hidden mb-2 shrink-0 group">
        <div 
          ref={scrollRef}
          onScroll={handleScroll}
          className="flex h-full overflow-x-auto snap-x snap-mandatory scrollbar-hide"
        ...
        {/* Pagination Dots */}
        {hasMultipleImages && (
          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1 px-1.5 py-0.5 bg-black/20 backdrop-blur-md rounded-full shadow-lg border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            {displayImages.map((_: any, idx: number) => (
              <div 
                key={idx} 
                className={`w-0.5 h-0.5 rounded-full transition-all duration-300 ${
        ...
        {/* Overlay Action */}
        <div className="absolute top-2 right-2 flex flex-col gap-1.5 translate-x-12 group-hover:translate-x-0 transition-transform duration-300 z-10">
          <button 
            onClick={() => setIsLiked(!isLiked)}
            className={`w-7 h-7 rounded-full bg-white/95 dark:bg-slate-900/90 backdrop-blur-sm flex items-center justify-center shadow-md transition-all duration-300 active:scale-90 ${isLiked ? 'text-rose-500' : 'text-slate-400 hover:text-rose-500'}`}
          >
        ...
        {product.badge && (
          <div className="absolute top-2 left-2 bg-primary text-white px-2 py-0.5 rounded-full shadow-lg z-10">
            <span className="text-[7px] md:text-[8px] font-black uppercase tracking-wider">{product.badge}</span>
          </div>
        )}
      </div>
      ...
      <div className="px-1 flex flex-col flex-1">
        <div className="mb-1">
          <span className="text-[7px] md:text-[8px] font-black uppercase tracking-[0.1em] text-primary">
            {product.category}
          </span>
        </div>
        ...
        <div className="mt-auto pt-2 border-t border-dashed border-slate-200 dark:border-white/10">
          <div className="flex items-center justify-between gap-1">
            <div className="flex flex-col">
              <span className="text-sm md:text-base font-black text-slate-950 dark:text-white tracking-tighter leading-none">
                ₹{product.price.toLocaleString()}
              </span>
              {product.originalPrice && (
                <div className="flex items-center gap-1 mt-0.5">
